...
subroutine inicia
    ...
    implicit none
    integer :: i,j,k
    character(20) :: nome_input

    print*, 'Arquivo de entrada: '
    read(*, *) nome_input

    open(1,file=trim(nome_input))
...
subroutine momentos
    ...
    character(20) :: nome_saida1, nome_saida2

    allocate(d1(N_mc),d4(N_mc))!,d2(N_mc),d3(N_mc),d4(N_mc))

    k = 0
    print*, 'Arquivo saída Energia: '
    read(*, *) nome_saida1
    print*, 'Arquivo saída Magnet: '
    read(*, *) nome_saida2

    open(3,file=trim(nome_saida1))
    open(4,file=trim(nome_saida2))
